...
namespace UnitTests.api
{
    public class ParentTaskDetailsControllerTests
    {
        ...
        {
            //Arrange
            manageTask.Setup(task => task.GetAll()).Returns(Task.FromResult<IEnumerable<ParentTaskDetail>>(TestData.GetParentTaskDetails()));
            ...
            Assert.Equal(((result as OkObjectResult).Value as List<ParentTaskDetail>).Count, TestData.GetParentTaskDetails().Count);
        }
        ...
        public async Task TestGetReturnsExpectedResults()
        {
            ...
            //Arrange
            var parentTaskDetail = ((TestData.GetParentTaskDetails()) as List<ParentTaskDetail>).Find(x => x.ParentId == 112);


            manageTask.Setup(x => x.Get(112)).Returns(Task.FromResult<ParentTaskDetail>(parentTaskDetail));
            ...
            Assert.Equal(typeof(OkObjectResult), result.GetType());
            Assert.NotNull((result as OkObjectResult).Value);
            Assert.Equal(112, ((result as OkObjectResult).Value as ParentTaskDetail).ParentId);
            Assert.Equal(parentTaskDetail.ParentTask, ((result as OkObjectResult).Value as ParentTaskDetail).ParentTask);
        }        
        ...
        public async Task TestDeleteMethodReturnsExpectedResult()
        {
            ...
            manageTask.Setup(x => x.Get(parentTaskDetail.ParentId)).Returns(Task.FromResult<ParentTaskDetail>(parentTaskDetail));
